...
const CreateSchema = z.object({
    tabla: z.string().min(1).max(50),
    registro_id: z.number().int().min(1),
...
/** Email HTML a administradores (MISMO DISEÑO que edición, texto adaptado) */
function buildHtmlEmailInact(opts: {
    solicitudId: number;
    tabla: string;
    registroId: number;
...
<title>Nueva Solicitud de Inactivación/Activación</title>
...
<h1 style="color:#fff;margin:0;font-size:24px;font-weight:700;letter-spacing:-.5px">Nueva Solicitud de Inactivación/Activación</h1>
...
<p style="margin:0;color:#78350f;font-size:13px;line-height:1.6">Ingresa al panel de administración para aprobar o rechazar esta solicitud.</p>
</td></tr></table></div>
<div style="background:#f8fafc;border-radius:8px;padding:16px;text-align:center"><p style="margin:0;color:#64748b;font-size:12px;line-height:1.6">Este correo es generado automáticamente por el sistema. No respondas este mensaje.</p></div>
</td></tr>
...
export async function POST(req: NextRequest) {
    ...
    const json = { ...raw, registro_id: typeof raw?.registro_id === 'string' ? Number(raw.registro_id) : raw?.registro_id };
    ...
    const recipients = adminEmails.length ? adminEmails : ['soporteusuariosbenjamin@gmail.com', 'cipallo@uce.edu.ec'];

    const client = await pool.connect();
    try {
        // Evitar duplicados PENDIENTE
        const dup = await client.query(
            `SELECT * FROM solicitudes_inactivacion
       WHERE tabla=$1 AND registro_id=$2 AND operador_id=$3 AND estado='PENDIENTE'
       ORDER BY creado_en DESC LIMIT 1`,
            [tabla, registro_id, user.id]
        );
        if (dup.rowCount > 0) {
            return NextResponse.json({ success: true, pending: true, data: dup.rows[0] }, { status: 200 });
        }

        await client.query('BEGIN');
        const ins = await client.query(
            `INSERT INTO solicitudes_inactivacion (tabla, registro_id, operador_id, motivo, estado, creado_en)
       ...
       VALUES ($1,'solicitudes_inactivacion',$2,'INSERT',$3::jsonb)`,
        ...
        // Email a administradores (MISMO asunto formateado)
        const transporter = makeTransporter();
        const operadorNombre = user?.nombre ?? user?.email;
        const html = buildHtmlEmailInact({
        ...
        if (transporter) {
            try {
                ...
                console.error('Error enviando correo de nueva solicitud (inactivación)', mailErr);
                emailWarning = true;
            }
        } else {
            console.log('--- EMAIL (dev) : nueva solicitud INACTIVACIÓN ---');
            console.log('To:', recipients.join(','));
            console.log('Subject:', subject);
            console.log('HTML:', html);
        }

        return NextResponse.json({ success: true, data: ins.rows[0], emailWarning }, { status: 201 });
    } catch (e) {
        try { await client.query('ROLLBACK'); } catch {}
        console.error('POST /api/inactivation-requests error', e);
        return NextResponse.json({ success: false, msg: 'Error interno al crear solicitud' }, { status: 500 });
    } finally {
        client.release();
    }
}
...
export async function GET(req: NextRequest) {
    ...
    if (operador_id) { params.push(Number(operador_id)); where += ` AND s.operador_id = $${params.length}`; }
    if (q.length) {
        params.push(`%${q}%`); params.push(`%${q}%`); params.push(`%${q}%`);
        where += ` AND (s.motivo ILIKE $${params.length-2} OR s.tabla ILIKE $${params.length-1} OR u.nombre ILIKE $${params.length})`;
    }

    const countSql = `SELECT COUNT(*) AS total
                    FROM solicitudes_inactivacion s LEFT JOIN usuarios u ON u.id=s.operador_id ${where}`;
    const dataSql = `
    SELECT s.*, u.nombre AS operador_nombre, u.email AS operador_email, ua.nombre AS aprobado_nombre
  ...
  `;
    ...
    try {
        ...
        console.error('GET /api/inactivation-requests error', e);
        return NextResponse.json({ success: false, msg: 'Error interno al listar' }, { status: 500 });
    } finally {
        client.release();
    }
}
