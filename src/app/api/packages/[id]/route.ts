...
export async function PUT(req: NextRequest, context: any) {
    ...
    try {
        ...
        const before = await client.query(`SELECT id, nombre, active FROM tipos_paquete WHERE id=$1`, [id]);
        ...
        const conflict = await client.query(
            `SELECT id FROM tipos_paquete WHERE active = TRUE AND LOWER(nombre) = LOWER($1) AND id != $2 LIMIT 1`,
            [nombre.trim(), id]
        );
        ...
        const upd = await client.query(
            `UPDATE tipos_paquete SET nombre=$1 WHERE id=$2 RETURNING id, nombre, active`,
            [nombre.trim(), id]
        );

        await client.query(
            `INSERT INTO auditoria (usuario_id, tabla, registro_id, accion, detalle)
             VALUES ($1,'tipos_paquete',$2,'UPDATE',$3::jsonb)`,
        ...
        console.error('PUT packages error', e);
...
export async function DELETE(req: NextRequest, context: any) {
    ...
    try {
        ...
        await client.query(`UPDATE tipos_paquete SET active=FALSE WHERE id=$1`, [id]);

        await client.query(
            `INSERT INTO auditoria (usuario_id, tabla, registro_id, accion, detalle)
             VALUES ($1,'tipos_paquete',$2,'DELETE',$3::jsonb)`,
        ...
        await client.query('ROLLBACK');
        console.error('DELETE packages error', e);
        return NextResponse.json({ success: false, msg: 'Internal error' }, { status: 500 });
    } finally {
        client.release();
    }
...
// PATCH /api/packages/:id  (activar/restaurar)
export async function PATCH(req: NextRequest, context: any) {
    ...
    try {
        ...
        const upd = await client.query(
            `UPDATE tipos_paquete SET active=$1 WHERE id=$2 RETURNING id, nombre, active`,
        ...
        console.error('PATCH packages error', e);
        return NextResponse.json({ success: false, msg: 'Internal error' }, { status: 500 });
    } finally {
        client.release();
    }
