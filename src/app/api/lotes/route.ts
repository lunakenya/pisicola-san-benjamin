...
// GET /api/lotes?q=&page=&pageSize=&includeInactive=
export async function GET(req: NextRequest) {
    ...
    const countSql = `SELECT COUNT(*) AS total FROM lotes p ${where}`;
    const dataSql = `
    ...
    try {
        ...
        console.error('GET lotes error', e);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const conflictActive = await client.query(
            `SELECT id FROM lotes WHERE active = TRUE AND LOWER(nombre) = LOWER($1) LIMIT 1`,
            [nombre.trim()]
        );
        ...
        const conflictInactive = await client.query(
            `SELECT id FROM lotes WHERE active = FALSE AND LOWER(nombre) = LOWER($1) LIMIT 1`,
            [nombre.trim()]
        );
        ...
        const ins = await client.query(
            `INSERT INTO lotes (nombre, active)
             VALUES ($1, TRUE)
        ...
        await client.query(
            `INSERT INTO auditoria (usuario_id, tabla, registro_id, accion, detalle)
             VALUES ($1,'lotes',$2,'INSERT',$3::jsonb)`,
        ...
        console.error('POST lotes error', e);
        return NextResponse.json({ success: false, msg: 'Internal error' }, { status: 500 });
    } finally {
        client.release();
    }
