// src/app/api/food-types/[id]/route.ts
...
export async function PUT(req: NextRequest, context: any) {
    ...
    try {
        ...
        const before = await client.query(`SELECT id, nombre, active FROM tipos_alimento WHERE id=$1`, [id]);
        ...
        const conflict = await client.query(
            `SELECT id FROM tipos_alimento WHERE active = TRUE AND LOWER(nombre) = LOWER($1) AND id != $2 LIMIT 1`,
            [nombre.trim(), id]
        );
        ...
        const upd = await client.query(
            `UPDATE tipos_alimento SET nombre=$1 WHERE id=$2 RETURNING id, nombre, active`,
            [nombre.trim(), id]
        );

        await client.query(
            `INSERT INTO auditoria (usuario_id, tabla, registro_id, accion, detalle)
             VALUES ($1,'tipos_alimento',$2,'UPDATE',$3::jsonb)`,
        ...
        console.error('PUT food-types error', e);
...
// DELETE /api/food-types/:id  (soft delete)
export async function DELETE(req: NextRequest, context: any) {
    ...
    try {
        ...
        await client.query(`UPDATE tipos_alimento SET active=FALSE WHERE id=$1`, [id]);

        await client.query(
            `INSERT INTO auditoria (usuario_id, tabla, registro_id, accion, detalle)
             VALUES ($1,'tipos_alimento',$2,'DELETE',$3::jsonb)`,
        ...
        await client.query('ROLLBACK');
        console.error('DELETE food-types error', e);
        return NextResponse.json({ success: false, msg: 'Internal error' }, { status: 500 });
    } finally {
        client.release();
    }
...
// PATCH /api/food-types/:id  (restore active=true/false)
export async function PATCH(req: NextRequest, context: any) {
    ...
    try {
        ...
        const upd = await client.query(
            `UPDATE tipos_alimento SET active=$1 WHERE id=$2 RETURNING id, nombre, active`,
        ...
        console.error('PATCH food-types error', e);
        return NextResponse.json({ success: false, msg: 'Internal error' }, { status: 500 });
    } finally {
        client.release();
    }
