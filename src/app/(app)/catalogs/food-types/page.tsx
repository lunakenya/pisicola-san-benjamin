// src/app/(app)/catalogs/food-types/page.tsx
'use client';
...
import styles from './food-types.module.css';

type FoodType = { id: number; nombre: string; active: boolean };
...
/* ---------- Modal Crear/Editar ---------- */
function FoodTypeFormModal({
                               open, initial, onClose, onSaved,
                           }: {
    open: boolean;
    initial?: Partial<FoodType>;
    onClose: () => void;
    onSaved: () => void;
}) {
    ...
    const submit = async (e?: React.FormEvent) => {
        ...
        try {
            if (isEdit && initial?.id) {
                const res = await fetch(`/api/food-types/${initial.id}`, {
                    method: 'PUT', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ nombre: nombre.trim() }),
                });
                await parseResponseOrThrow(res);
                Toast.fire({ icon: 'success', title: 'Tipo de alimento actualizado' });
            } else {
                ...
                await parseResponseOrThrow(res);
                Toast.fire({ icon: 'success', title: 'Tipo de alimento creado' });
            }
            onSaved();
            onClose();
        } catch (err: any) {
    ...
    return (
        <div className={styles.modalOverlay} role="dialog" aria-modal="true">
            <form className={styles.modal} onSubmit={submit}>
                <h3 className={styles.modalTitle}>{isEdit ? 'Editar tipo de alimento' : 'Nuevo tipo de alimento'}</h3>
...
export default function FoodTypesPage() {
    ...
    const [formInitial, setFormInitial] = useState<Partial<FoodType> | undefined>(undefined);
    ...
    async function fetchData({ initial = false }: { initial?: boolean } = {}) {
        ...
        try {
            ...
            const url = `/api/food-types?${params.toString()}`;
            ...
            if (json?.success) {
                ...
                Toast.fire({ icon: 'error', title: json?.msg ?? 'Error al cargar tipos de alimento' });
            }
        } catch (e: any) {
            if (e?.name !== 'AbortError') {
                console.error(e);
                setData([]); setTotal(0); setPages(1);
                Toast.fire({ icon: 'error', title: 'Error al cargar tipos de alimento' });
            }
        } finally {
    ...
    const onEdit = (p: FoodType) => { setFormInitial(p); setOpenForm(true); };
    const onSaved = () => { void fetchData(); };

    // inactivar/restaurar
    const onAskToggleActive = async (p: FoodType) => {
        const action = p.active ? 'Inactivar' : 'Restaurar';
        const result = await Swal.fire({
            title: `${action} tipo de alimento`,
            html: p.active
                ? `¿Inactivar el tipo de alimento "<b>${p.nombre}</b>"?`
                : `¿Restaurar el tipo de alimento "<b>${p.nombre}</b>"?`,
            icon: 'warning',
        ...
        try {
            if (p.active) {
                const res = await fetch(`/api/food-types/${p.id}`, { method: 'DELETE' });
                await parseResponseOrThrow(res);
                Toast.fire({ icon: 'success', title: `Tipo de alimento "${p.nombre}" inactivado` });
            } else {
                const res = await fetch(`/api/food-types/${p.id}`, {
                    method: 'PATCH', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ active: true }),
                });
                await parseResponseOrThrow(res);
                Toast.fire({ icon: 'success', title: `Tipo de alimento "${p.nombre}" restaurado` });
            }
            void fetchData();
        } catch (e: any) {
            console.error(e);
        ...
        <div className={styles.wrap}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    <div className={styles.searchBox}>
                        ...
                        <input
                            ...
                            aria-label="Buscar tipos de alimento"
                        />
                    </div>
          ...
          <span className={styles.counter}>
            {initialLoading && total === 0 ? '—' : `${from}–${to} de ${total}`}
                    ...
                    <button className={styles.roundButton} title="Nuevo tipo" onClick={onCreate} aria-label="Nuevo tipo de alimento">
                        <FiPlus />
                    </button>
                </div>
            </div>
            ...
            <FoodTypeFormModal open={openForm} initial={formInitial} onClose={() => setOpenForm(false)} onSaved={onSaved} />
        </div>
    );
}
